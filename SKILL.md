...
description: 为代码仓生成、查询和维护面向人类与 Agent 的深度分层中文知识库。用于初始化或升级 `.repo-knowledge/`，按“项目总览→前后端等子系统总览→独立业务模块目录→模块手册/专题→源码与测试”渐进理解业务。每个模块必须拥有自己的目录和完整开发手册；任务结束前必须清除所有占位词并通过硬性质量门禁。前端覆盖用户流程、View、重要组件、状态、交互和 API 协作；后端覆盖业务用例、规则分支、实现算法、数据、事务、并发和副作用。也用于需求完成或未知代码改动后同步知识；适用于单体、多模块、前后端同仓及多语言项目。
---
...
1. 建立严格的分层导航：`INDEX.md` → `systems/<子系统>/overview.md` → `systems/<子系统>/modules/<模块>/overview.md` → 模块专题（需要时）→ 源码与测试。
...
3. 先读 `systems/<子系统>/modules/<模块>/overview.md`，再按问题读取该目录链接的业务、实现、接口或开发专题。
...
10. 每个模块使用独立目录。`overview.md` 必须自洽并链接该模块的其他专题；较大模块可增加 `business.md`、`implementation.md`、`interfaces.md`、`development.md` 等，但不得把专题散落到模块目录之外。
11. 回填子系统总览的业务旅程、模块关系、模块职责、接口/路由/别名检索词，再回填根总览的跨系统业务链路和新人阅读路径。
12. 运行 `scan --update` 仅刷新机械清单和补建缺失骨架，不指望脚本替代源码调查。扫描可能发现新模块，因此必须在它之后做最终清理。
13. 运行 `doctor --strict`。v4 中警告也会无条件导致失败；它会扫描全部 Markdown，拦截扁平模块、占位词、短文档、浅章节、无编号流程、前端无组件树、后端无规则表和证据不足。
14. 若失败，逐项回到源码调查并补写，重复 `scan --update` → `doctor --strict`，直到输出“错误 0，警告 0”。禁止通过删除章节、删除证据或把占位词替换成模糊空话来通过。
15. 再独立运行 `rg -n "待补充|待调查|待核对|KNOWLEDGE_TODO|TODO|TBD" .repo-knowledge -g "*.md"`；必须没有输出。然后执行新人上手验收，任一步未通过都不能结束任务。

升级现有知识库时先读 `inventory/schema-version`。v1/v2 先按既有规则迁移；v3 的 `modules/<模块>.md` 必须运行 `upgrade-layout` 迁入 `modules/<模块>/overview.md`，随后更新所有链接并按 v4 门禁清除占位内容。确认新导航和新人开发手册完整后才能移除旧入口。
